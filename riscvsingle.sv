...
module controller(input  logic [6:0] op,
                  ...
                  output logic       do_mmatmul, do_mload, do_mstore);
...
module maindec(input  logic [6:0] op,
  ...
  assign {RegWrite, ImmSrc, ALUSrc, MemWrite,
          ...
          do_mmatmul, do_mload, do_mstore} = controls;
  /*
  Mul instruction
  ...
  000 for mul, 001 for mulh, 010 for mulhsu, 011 for mulhu

  Custom MAC instruction
  [31:25]   [24:20]  [19:15]  [14:12]  [11:7]   [6:0]
  funct7     rs2      rs1      funct3    rd      opcode

  0000000    rs2      rs1       000      rd     0001011    ← mac rd, rs1, rs2

  Custom dot4 instruction
  [31:25]   [24:20]  [19:15]  [14:12]  [11:7]   [6:0]
  funct7     rs2      rs1      funct3    rd      opcode

  0000000    rs2      rs1       001      rd     0001011    ← dot4 rd, rs1, rs2

  Custom mmatmul instruction (outer product — updates 16 coprocessor accumulators)
  [31:25]   [24:20]  [19:15]  [14:12]  [11:7]   [6:0]
  funct7     rs2      rs1      funct3    rd      opcode

  0000000    rs2      rs1       010      xxxxx  0001011    ← mmatmul rs1, rs2

  Custom mload instruction (load 32-bit value into coprocessor C register)
  [31:20]       [19:15]  [14:12]  [11:7]   [6:0]
  imm[11:0]     rs1      funct3   c_reg    opcode

  imm[11:0]     rs1       000     c_reg   0101011    ← mload c_reg, imm(rs1)

  Custom mstore instruction (store coprocessor C register to memory)
  [31:25]   [24:20]  [19:15]  [14:12]  [11:7]    [6:0]
  imm[11:5]  c_reg   rs1      funct3   imm[4:0]  opcode

  imm[11:5]  c_reg   rs1       010     imm[4:0] 0101011    ← mstore c_reg, imm(rs1)
  */

  always_comb
    case(op)
    // RegWrite_ImmSrc_ALUSrc_MemWrite_ResultSrc_Branch_ALUOp_Jump_mmatmul_mload_mstore
      7'b0000011: controls = 14'b1_00_1_0_01_0_00_0_0_0_0; // lw
      7'b0100011: controls = 14'b0_01_1_1_00_0_00_0_0_0_0; // sw
      7'b0110011: controls = 14'b1_xx_0_0_00_0_10_0_0_0_0; // R-type
      7'b1100011: controls = 14'b0_10_0_0_00_1_01_0_0_0_0; // beq
      7'b0010011: controls = 14'b1_00_1_0_00_0_10_0_0_0_0; // I-type ALU
      7'b1101111: controls = 14'b1_11_0_0_10_0_00_1_0_0_0; // jal
      7'b0001011: begin // custom-0: mac, dot4, mmatmul (differentiated by funct3)
        case(funct3)
          3'b000:  controls = 14'b1_xx_0_0_11_0_00_0_0_0_0; // mac
          3'b001:  controls = 14'b1_xx_0_0_11_0_00_0_0_0_0; // dot4
          3'b010:  controls = 14'b0_xx_0_0_xx_0_00_0_1_0_0; // mmatmul (outer product)
          default: controls = 14'bx_xx_x_x_xx_x_xx_x_0_0_0;
        endcase
      end
      7'b0101011: begin // custom-1: mload, mstore (differentiated by funct3)
        case(funct3)
          3'b000:  controls = 14'b0_00_1_0_xx_0_00_0_0_1_0; // mload (I-type addr calc)
          3'b010:  controls = 14'b0_01_1_1_xx_0_00_0_0_0_1; // mstore (S-type addr calc)
          default: controls = 14'bx_xx_x_x_xx_x_xx_x_0_0_0;
        endcase
      end
      default:    controls = 14'bx_xx_x_x_xx_x_xx_x_0_0_0; // non-implemented instruction
    endcase
endmodule
...
module datapath(input  logic        clk, reset,
                ...
                output logic [31:0] ALU_MULResult, WriteData,
                input  logic [31:0] ReadData,
                input  logic        do_mmatmul, do_mload, do_mstore);
  ...
  logic [31:0] MAC_DOT4_result;   // rd + rs1 * rs2
  ...
  logic mode;
  always_comb begin
    if (op == 7'b0001011 && funct3 == 3'b001)
      mode = 1'b0;                        // dot4
    else if (op == 7'b0001011 && funct3 == 3'b000)
      mode = 1'b1;                        // mac
    else
      mode = 1'b0;                     
  end
  ...
  mac_dot4  mac_dot4(SrcA, SrcB, rd_current, mode, MULResult, MAC_DOT4_result);
  mux4 #(32)  resultmux(ALU_MULResult, ReadData, PCPlus4E, MAC_DOT4_result, ResultSrc, Result);

  // ===== OUTER PRODUCT COPROCESSOR =====
  // C register index: lower 4 bits of rd field for mload, lower 4 bits of rs2 field for mstore
  ...
  // WriteData mux: coprocessor C register for mstore, else rd2 from register file
